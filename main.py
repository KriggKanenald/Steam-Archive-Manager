...
class SteamManagerApp:
    ...
    def create_main_content(self) -> None:
        ...
        scrollbar = tk.Scrollbar(
            body_frame,
            orient="vertical",
            command=self.game_canvas.yview,
        )
        ...
        self.rows_frame.columnconfigure(4, weight=1)
        ...
        self.rows_frame.bind(
            "<Configure>",
            lambda event: self.game_canvas.configure(
                scrollregion=self.game_canvas.bbox("all")
            ),
        )
        self.game_canvas.bind(
            "<Configure>",
            lambda event: self.game_canvas.itemconfigure(
                self.rows_window,
                width=event.width,
            ),
        )
    ...
    def refresh_game_list(self) -> None:
        ...
        self.path_status_label.config(
            text=self.t("status_steam_path", steam_path=steam_path)
        )

        try:
            games = self.find_installed_games(steam_path)
        except FileNotFoundError as error:
            self.show_game_message(str(error))
            return
        except OSError as error:
            self.show_game_message(self.t("message_read_steam_error", error=error))
            return
        ...
        for game in games_only:
            self.add_game_row(game, row_index)
            row_index += 1

        if tools:
            self.add_section_separator(self.t("section_tools"), row_index)
            row_index += 1
            for tool in tools:
                self.add_game_row(tool, row_index)
                row_index += 1
    ...
    def add_table_header(self) -> None:
        ...
        self.add_column_separator(self.rows_frame, row_index, 1)
        self.add_cell_label(
            self.rows_frame,
            row_index,
            2,
        ...
        self.add_column_separator(self.rows_frame, row_index, 3)
        self.add_cell_label(
            self.rows_frame,
            row_index,
            4,
        ...
        self.add_column_separator(self.rows_frame, row_index, 5)
        self.add_cell_label(
            self.rows_frame,
            row_index,
            6,
        ...
        self.add_column_separator(self.rows_frame, row_index, 7)
        self.add_cell_label(
            self.rows_frame,
            row_index,
            8,
        ...
        self.add_column_separator(self.rows_frame, row_index, 9)
        self.add_cell_label(
            self.rows_frame,
            row_index,
            10,
        ...
        self.add_column_separator(self.rows_frame, row_index, 11)
        self.add_cell_label(
            self.rows_frame,
            row_index,
            12,
        ...
        self.add_column_separator(self.rows_frame, row_index, 13)
        self.add_cell_label(
            self.rows_frame,
            row_index,
            14,
    ...
    def wait_for_steam_update(
        ...
        while time.monotonic() < deadline:
            ...
            manifest_target_version = manifest.get("targetbuildid", "").strip()
            if manifest_target_version and manifest_target_version != "0":
    ...
    def show_game_message(self, message: str) -> None:
        tk.Label(
            ...
            fg="#555555",
            pady=24,
        ).grid(row=1, column=0, columnspan=15, sticky="ew")
    ...
    def add_game_row(self, game: SteamGame, index: int) -> None:
        row_background = "#ffffff" if index % 2 == 0 else "#f7f7f7"

        selected = tk.BooleanVar(value=False)
        self.game_selection[str(game.folder)] = selected
    ...
    def load_header_image(self, header_path: Path | None) -> tk.PhotoImage | None:
        ...
        if Image is not None and ImageTk is not None:
            try:
                ...
                image.thumbnail(HEADER_IMAGE_SIZE)
    ...
    def get_latest_version_text(
        ...
        latest_version = app_metadata.get("latest_version", "")
        if latest_version:
            return latest_version

        return manifest.get("targetbuildid") or UNAVAILABLE_VERSION
    ...
    def parse_appmanifest(self, manifest_path: Path) -> dict[str, str]:
        ...
        for line in content.splitlines():
            ...
            if match:
                manifest[match.group(1).casefold()] = match.group(2)

        return manifest

    def find_header_path(self, steam_path: Path, app_id: str) -> Path | None:
        library_cache_path = steam_path / "appcache" / "librarycache" / app_id
